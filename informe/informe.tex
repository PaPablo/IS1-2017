...
\begin{document}
    ...
    \subsection{Productos}
        ...
        Para las reparaciones, mantenimiento de equipos e instalaciones, la organización dispone de un stock de productos (insumos) necesarios para el desarrollo de esas actividades (var seccion 2.3.1).
    
    	Los productos se clasifican en partes (engranajes de impresoras, mecanismos de tiqueadoras, memorias fiscales, cables de datos, cables de alimentación) y componentes (discos duros, memorias, placas madres, lecto-grabadoras de CD-DVD, teclados, mouses).\\
    ...
    \begin{itemize}
        ...
        \item proveedor/es (teniendo como primera opción al mas usual. \footnote{Es importante tener en cuenta que hay clientes corporativos que a la vez pueden ser proveedores. Un ejemplo de esto es EPSON ya que provee insumos para la reparación de las impresoras fiscales}. 
		\begin{itemize}
    ...
    Los productos descriptos en el punto anterior son proveídos por diferentes empresas. El stock se actualiza cuando es necesario (on-demand). Sin embargo, se intenta mantener una cantidad básica de cada uno de los elementos.\\
	...
	Todos los empleados tienen autorización para mirar el stock de productos. Si algún empleado considera que no hay suficiente stock de algún/os producto/s, se le comunica al Gerente que es quien decide ordenar (o no) al Operario Contable que realice un pedido. Todos los pedidos son realizados por el Operario Contable vía correo electrónico o en sitios de venta online. Éste, al realizar el pedido, debe registrar a qué proveedor se hizo, qué artículos y a qué precio, fecha en la que se hizo y fecha probable de llegada. Este registro, al igual que las recepciones y los pagos, se hace en una hoja de cálculos.
    ...
    El proceso de reparación comienza cuando se acerca un cliente a la oficina de la empresa con algún equipo a reparar. El técnico que atiende al cliente verifica si está cargado en el sistema; si no es así, se le toman los datos y se lo registra. Si el cliente ya estaba cargado y era moroso se debe considerar prestar o no el servicio (decisión tomada por el Gerente).
    
    Una vez cargado, el técnico abre una \textit{Orden de Trabajo} y se asigna a sí mismo como el que atendió la solicitud y encargado de la \textit{Orden de Trabajo}. La \textit{Orden de Trabajo} pasa a estado \textit{Asignada} (para ver los distintos estados de una \textit{Orden de Trabajo}, ver Anexo 2). 
   
    En la \textit{Orden de Trabajo} también se va a registrar:
    \begin{itemize}
        \item La fecha de solicitud de servicio
        \item El cliente solicitante
        \item Una descripción que dio el cliente con respecto al problema
        \item Tipo de equipo de que se trate con marca, modelo y número de serie asignado
        \item Vencimiento (tentativo, de la fecha de creación de la \textit{Orden de Trabajo})
    \end{itemize}

    Toma el equipo (ya sea PC, notebook, Tablet, impresora láser) y verifica si se encuentra en existencia en el sistema. De no ser así se lo carga registrando marca y modelo. Cada equipo que ingresa al taller se le asigna un número de serie generado automáticamente por el sistema, de modo que un cliente puede tener asociado varios equipos, contando cada uno con su historial de reparaciones. Luego de que ya fue registrado, se vincula el equipo a la \textit{Orden de Trabajo}
   %quién puede abrir OT??????????? 
